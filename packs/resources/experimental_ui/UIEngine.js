...
(function(factory) {
    ...
    Emitter.prototype._createClear = function(object, name, handler) {
        return function() {
            const handlers = object.events[name];
            if (handlers) {
                let index = -1;
    ...
    Emitter.prototype.on = function(name, callback, context) {
        ...
        const handler = new Handler(callback, context || this);
    ...
    const isAttached = engine !== undefined;
    engine = engine || {};
    ...
    Promise.prototype.merge = function(other) {
        if (this.state === pending) {
            ...
            const handlers = other.emitter.events[this.state];
            const self = this;
            if (handlers !== undefined) {
                handlers.forEach(function(handler) {
    ...
    Promise.prototype.then = function(callback, errback) {
        ...
        const handler = new Handler(callback || makeDefaultHandler(this), this);

        this.success(this.make_chain(handler, promise, fulfilled), this);

        const errorHandler = new Handler(errback || makeDefaultHandler(this), this);
    ...
    if (!engine.isAttached) {
        ...
        engine.SendMessage = function(name, id) {
            ...
            const deferred = engine._ActiveRequests[id];

            delete engine._ActiveRequests[id];
            const call = function() {
                const mock = engine._mocks[name];
        ...
        engine.TriggerEvent = function() {
            ...
            const trigger = function() {
                const mock = engine._mocks[args[0]];
        ...
        engine._mockImpl = function(name, mock, isCppCall, isEvent) {
            ...
            const functionStripped = mock.toString().replace(`function ${mock.name}(`, "");
            const rightParanthesis = functionStripped.indexOf(")");
            const args = functionStripped.substr(0, rightParanthesis);
    ...
    engine.events = {};
    for (const property in Emitter.prototype) {
    ...
    if (engine.isAttached) {
        engine.on = function(name, callback, context) {
            ...
            if (handlers === undefined && engine.AddOrRemoveOnHandler !== undefined) {
                ...
                const prevEvent = engine.AddOrRemoveOnHandler(name, callback, context || engine);
                ...
                const prevHandler = new Handler(prevEvent[0], prevEvent[1] || this);
    ...
    const concatArguments = Array.prototype.concat;
    engine.trigger = function(name) {
        this._trigger.apply(this, arguments);
        this.TriggerEvent.apply(this, arguments);

        if (this.events["all"] !== undefined) {
            const allArguments = concatArguments.apply(["all"], arguments);
    ...
    engine.createDeferred =
        global.engineCreateDeferred === undefined
            ? function() {
                  return new Promise();
              }
    ...
    engine.call = function() {
        engine._RequestId++;
        const id = engine._RequestId;

        const deferred = engine.createDeferred();
        engine._ActiveRequests[id] = deferred;
        const messageArguments = Array.prototype.slice.call(arguments);
    ...
    engine._Result = function(requestId) {
        const deferred = engine._ActiveRequests[requestId];
        if (deferred !== undefined) {
            delete engine._ActiveRequests[requestId];

            const resultArguments = Array.prototype.slice.call(arguments);
    ...
    engine._Register = function(eventName) {
        const trigger = (function(name, engine) {
            return function() {
                const eventArguments = [name];
    ...
    engine._removeEventThunk = function(name) {
        const handle = engine._eventHandles[name];
        handle.clear();
